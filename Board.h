...
public:
	...
	/* gets the main word the player wanted to create */
	void getWord(std::vector<std::pair<std::string, unsigned int>>& words ,
	const PlaceMove& m, Square* curr, size_t col, size_t row, char letter) const;

	/* gets a word formed vertically when you place something down
	horiontally and vise versa */
	void getWordHelper(std::vector<std::pair<std::string, unsigned int>>& words ,
	...
	*/
	...
	/* gets startX of the start square */
	size_t getStartX() const {return startX;}

	/* gets startY of the start square */
	size_t getStartY() const {return startY;}
	...
	/* returns a pointer to the start square*/
	Square* getStart() const {return getSquare(startX, startY);}
